...
public class identificadoModificarTestigo extends JPanel {
	...
	public identificadoModificarTestigo(Identificado ident) {
		...
		btnVincularTestigo.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				boolean seVincula = identificadoDao.agregarTestigoAlIdentificado(identificado, testigoAVincular.getDniTestigo());
	...
	private void datosTestigosDisponibles(Identificado ident) {
        ...
        if (ident.getTestigoDelDetenido()!=null) {
            ...
            for (Testigo t : TestigosDisponibles) {
            	...
            	String nombre = t.getNombreCompleto();
            	String testimonio = t.getTestimonio();
                 if (tDAO.conseguirIDTestigo(t.getDniTestigo())!=identificado.getDniDetenido()) {
                        modelTestigosDisponibles.addRow(new Object[] { dni, nombre, testimonio });
	...
	private void datosTestigoQueYaTiene(Identificado ident) {
		...
		modelTestigoQueYaTiene.setRowCount(0);
			int dni = identificado.getTestigoDelDetenido().getDniTestigo();
			String nombre = identificado.getTestigoDelDetenido().getNombreCompleto();
			String testimonio = identificado.getTestigoDelDetenido().getTestimonio();
			modelTestigoQueYaTiene.addRow(new Object[] {
					dni,nombre,testimonio
				});
			System.out.println("Testigo que ya fue agregado: " + identificado.getTestigoDelDetenido().getDniTestigo());//no se
		}
